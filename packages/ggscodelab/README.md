...
```bash
npx ggscodelab install-skill <skill-name> --cursor | --claude
```

**Update** (overwrites existing skill after confirmation, or use `--yes` to skip prompt):

```bash
npx ggscodelab update-skill <skill-name> --cursor | --claude [--yes]
```

**Examples:**

```bash
# Install dev-team-lead into Cursor (.cursor/skills/dev-team-lead/)
npx ggscodelab install-skill dev-team-lead --cursor

# Install dev-team-lead into Claude folder (claude/dev-team-lead/)
